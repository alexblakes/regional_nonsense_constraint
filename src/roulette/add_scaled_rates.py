...
def scale_by_background(vcf_dir, input_filename, background, denovo):
    ...
    if denovo == 0:
        ...
        def poisson_function(x):
            # Alternatively use the likelihood?
            # return -np.sum(binom.logpmf(k=df_group["polymorphic"], n=df_group["total"], p=(1 - np.exp(-x * df_group["mu"]))))
            # Keep this for now... check to make sure it doesn't do anything funny
        ...
        # This all seems fine, though the likelihood approach should work for both?

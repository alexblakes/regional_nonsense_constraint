...
dx rm -rf outputs/gnomad_fail_variants/
dx mkdir -p outputs/gnomad_fail_variants/split_fail_variants
...
dx upload --destination /outputs/gnomad_fail_variants/ "${ALL_PATHS}"
...
dnanexus-upload-agent-*-linux/ua --do-not-compress -f /outputs/gnomad_fail_variants/ split_paths
...
do
    ...
    -iin="${project}:/scripts/extract_fail_variants.sh" \
    -iin="${project}:/outputs/gnomad_fail_variants/split_paths/${FILE_NAME}" \
    -iin="${project}:/data/gencode_v39_canonical_cds_chr.bed" \
    -icmd="bash extract_fail_variants.sh" \
    --name="extract_fail_variants_original_230226 ${FILE_NAME}" \
    --instance-type="mem3_ssd1_v2_x8" \
    --destination="${project}:/outputs/gnomad_fail_variants/split_fail_variants/" \
    --priority="low" \
    --cost-limit 0.40 \
    --tag "original" \
    --tag "parallel" \
